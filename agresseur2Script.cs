...
public class agresseur2Script : MonoBehaviour
{
    ...
    void Start()
    {
      ...
      B2agentSetDestination = false;
    }

    // Update is called once per frame
    void Update()
    {
      if (B2agentSetDestination == true)
      {
        agent.SetDestination (joueur.transform.position);
      }

      if (B2agentSetDestination == false)
      {
    ...
    void OnTriggerStay (Collider other)
    {
      ...
      if (other.gameObject.CompareTag("Player") && other.gameObject.CompareTag("Ia1"))
      {
        vie.GetComponent<vieScript>().currentHP -= 10f;
      }
    }
